...
public class BeamTowerBlockEntity extends BaseGunTower {
    public BeamTowerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.BEAM_TOWER, pos, state);
    }
    ...
    protected void shoot(World world) {
        ...
        Vec3d towerPos = Vec3d.ofCenter(pos).add(0, 5, 0);
        Vec3d targetPos = t.getPos().add(0, t.getHeight(), 0);

        spawnBulletTrail(world, towerPos, targetPos);
        t.timeUntilRegen = 0;
        boolean died = t.damage(world.getDamageSources().generic(), 223.0f);
    ...
    protected static void spawnBulletTrail(World level, Vec3d start, Vec3d end) {
        ...
        Vec3d step = direction.normalize().multiply(0.2);
        int steps = (int)(length / 0.2);
        Vec3d pos = start;
        for (int i = 0; i < steps; i++) {
            ((ServerWorld) level).spawnParticles(ParticleTypes.GLOW, pos.x, pos.y, pos.z, 1, 0, 0, 0, 0);
            pos = pos.add(step);
        }
    }
}
