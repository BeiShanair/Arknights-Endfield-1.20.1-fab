...
public class GrenadeTowerBlockEntity extends BaseGunTower {

    public GrenadeTowerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.GRENADE_TOWER, pos, state);
    }
    ...
    protected void shoot(World world) {
        ...
        t.timeUntilRegen = 0;
        boolean died = t.damage(world.getDamageSources().generic(), 9.0f);
        ...
        for (LivingEntity e : world.getEntitiesByClass(LivingEntity.class, box, e -> e.isAlive() && !(e instanceof PlayerEntity))) {
            ...
            if (distance <= 5) {
                double damage = 9.0 * (1 - distance / 5);
                e.timeUntilRegen = 0;
