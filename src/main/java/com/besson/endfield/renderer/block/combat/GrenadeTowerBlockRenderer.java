...
public class GrenadeTowerBlockRenderer extends GeoBlockRenderer<GrenadeTowerBlockEntity> {
    public GrenadeTowerBlockRenderer(BlockEntityRendererFactory.Context context) {
        super(new GrenadeTowerModel());
    }

    @Override
    public void actuallyRender(MatrixStack poseStack, GrenadeTowerBlockEntity animatable, BakedGeoModel model, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
