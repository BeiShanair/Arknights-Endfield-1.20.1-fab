...
public class HeavyGunTowerBlock extends BlockWithEntity {
    public HeavyGunTowerBlock(Settings settings) {
        super(settings);
    }

    @Override
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HeavyGunTowerBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.HEAVY_GUN_TOWER, HeavyGunTowerBlockEntity::tick);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        tooltip.add(Text.translatable("endfield.range", 15).formatted(Formatting.GRAY));
        tooltip.add(Text.translatable("endfield.attack", 21).formatted(Formatting.GRAY));
