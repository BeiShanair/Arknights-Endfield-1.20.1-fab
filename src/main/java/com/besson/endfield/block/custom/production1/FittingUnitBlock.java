...
import com.besson.endfield.blockentity.custom.production1.FittingUnitSideBlockEntity;
...
public class FittingUnitBlock extends ModBlockEntityWithFacing {
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.FITTING_UNIT, FittingUnitBlockEntity::tick);
    }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        if (!world.isClient()) {
            ...
            for (BlockPos p : adjacentPositions) {
                world.setBlockState(p, ModBlocks.FITTING_UNIT_SIDE.getDefaultState().with(FACING, state.get(FACING)));
