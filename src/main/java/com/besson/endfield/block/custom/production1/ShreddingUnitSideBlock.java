...
import com.besson.endfield.blockentity.custom.production1.ShreddingUnitSideBlockEntity;
...
public class ShreddingUnitSideBlock extends ModBlockEntityWithFacing {
    ...
    public ShreddingUnitSideBlock(Settings settings) {
        super(settings);
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof ShreddingUnitSideBlockEntity sideBlock) {
                ShreddingUnitBlockEntity parent = sideBlock.getParentBlock();
                if (parent != null) {
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.SHREDDING_UNIT_SIDE, ShreddingUnitSideBlockEntity::tick);
    }

    @Override
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.SHREDDING_UNIT_ITEM);
    }
}
