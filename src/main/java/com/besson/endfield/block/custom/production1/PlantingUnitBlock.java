...
public class PlantingUnitBlock extends ModBlockEntityWithFacing {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient()) {
            NamedScreenHandlerFactory screenHandlerFactory = (PlantingUnitBlockEntity) world.getBlockEntity(pos);
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            for (BlockPos p : adjacentPositions) {
                if (world.getBlockState(p).getBlock() == ModBlocks.PLANTING_UNIT_SIDE) {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        if (!world.isClient()) {
            ...
            for (BlockPos p : sidePositions) {
                world.setBlockState(p, ModBlocks.PLANTING_UNIT_SIDE.getDefaultState().with(FACING, state.get(FACING)));
    ...
    private BlockPos[] getAdjacentPositions(BlockState state, BlockPos pos) {
        ...
        return new BlockPos[]{
                ...
                pos.offset(facing, 2),
                pos.offset(facing, 2).offset(left), pos.offset(facing, 2).offset(right),
                pos.offset(facing, 2).offset(left, 2), pos.offset(facing, 2).offset(right, 2),
