...
public class DepotUnloaderBlock extends ModBlockEntityWithFacing {
    public DepotUnloaderBlock(Settings settings) {
        super(settings);
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            for (BlockPos p : adjacentPositions) {
                if (world.getBlockState(p).getBlock() == ModBlocks.DEPOT_UNLOADER_SIDE) {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        if (!world.isClient()) {
            ...
            for (BlockPos p : sidePos) {
                world.setBlockState(p, ModBlocks.DEPOT_UNLOADER_SIDE.getDefaultState().with(FACING, state.get(FACING)));
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.DEPOT_UNLOADER, DepotUnloaderBlockEntity::tick);
    }
