...
public class GrindingUnitRecipeBuilder {
    private final List<ItemCountInput> inputs;
    private final ItemConvertible output;
    private final int outputCount;

    private GrindingUnitRecipeBuilder(List<ItemCountInput> inputs, ItemConvertible output, int outputCount) {
        this.inputs = inputs;
        this.output = output;
        this.outputCount = outputCount;
    }

    public static GrindingUnitRecipeBuilder create(List<ItemCountInput> inputs, ItemConvertible output) {
        return new GrindingUnitRecipeBuilder(inputs, output, 1);
    }

    public static GrindingUnitRecipeBuilder create(List<ItemCountInput> inputs, ItemConvertible output, int outputCount) {
        return new GrindingUnitRecipeBuilder(inputs, output, outputCount);
    }
    ...
    public void offerTo(Consumer<RecipeJsonProvider> exporter, Identifier id) {
        exporter.accept(new RecipeJsonProvider() {
            ...
            public void serialize(JsonObject json) {
                json.addProperty("type", ArknightsEndfield.MOD_ID + ":grinding_unit");
            ...
            public RecipeSerializer<?> getSerializer() {
                return GrindingUnitRecipe.Serializer.INSTANCE;
            }
